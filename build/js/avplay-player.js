    ...
    function play(url, options) {
        ...
        if (isTizen) {
            try {
                ...
                if (isHls && url.indexOf("|COMPONENT=HLS") === -1) {
                    url += "|COMPONENT=HLS";
                } else if (isDash && url.indexOf("|COMPONENT=DASH") === -1) {
                    url += "|COMPONENT=DASH";
                }

                if (options.customHeaders) {
                    var headerStrings = [];
                    Object.keys(options.customHeaders).forEach(function(k) {
                        headerStrings.push(k + "=" + options.customHeaders[k]);
                    });
                    if (headerStrings.length > 0) {
                        url += "|" + headerStrings.join("&");
                        log("Appended custom headers to URL: " + headerStrings.join("&"));
                    }
                }

                root.webapis.avplay.open(url);
                log("AVPlay open success: " + url);
